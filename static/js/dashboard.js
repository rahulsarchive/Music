...
(function () {
  ...
  // ---------- Per-chord trend ----------
  // Uses bpm_trend from /api/chords if present; falls back to a synthesized curve.
  function mockChordTrend(chord) {
    if (chord.bpm_trend && chord.bpm_trend.length) return chord.bpm_trend;
  ...
  // Uses accuracy_30d from /api/chords if present; falls back to time-based estimate.
  function mockChordAccuracy(chord) {
    if (chord.accuracy_30d != null) return chord.accuracy_30d;
    const secs = chord.total_practice_seconds || 0;
    if (secs === 0) return 0;
    const base = 0.55 + Math.min(0.4, secs / (60 * 60 * 3));
    const tagBoost = (chord.frets || []).some((f) => f > 4) ? -0.1 : 0;
  ...
  async function refresh(practice) {
    ...
    if (practice && practice.chords) {
      ...
      let plan;
      try {
        const planData = await fetch("/api/plan/today").then((r) => r.json());
        if (Array.isArray(planData) && planData.length) {
          plan = planData.map((item) => {
            if (item.kind === "chord") {
              const target = practice.chords.find((c) => c.id === item.target_id)
                || { id: item.target_id, display_name: item.target_name };
              return { ...item, target, title: item.target_name, goalMin: item.goal_min };
            } else {
              const target = (practice.progressions || []).find((p) => p.id === item.target_id)
                || { id: item.target_id, name: item.target_name };
              return { ...item, target, title: item.target_name, goalMin: item.goal_min };
            }
          });
        } else {
          plan = buildTodayPlan(practice.chords, practice.progressions || []);
        }
      } catch (_) {
        plan = buildTodayPlan(practice.chords, practice.progressions || []);
      }
